...
This directory (`my-mlops-project/terraform`) includes Databricks ML resource configuration, i.e. the definitions of a batch
...
the contents of `my-mlops-project/terraform` to the Git repo:
```
git checkout -b add-ml-resource-config
git add my-mlops-project/terraform
...
Then, resolve the TODOs in `my-mlops-project/terraform/staging/inference-job.tf` and `my-mlops-project/terraform/prod/inference-job.tf`, passing
...
To get started, open `my-mlops-project/terraform/staging/inference-job.tf`.  The file contains the ML resource definition of
...
resource "databricks_job" "batch_inference_job" {
  ...
  notebook_task {
    notebook_path = "my-mlops-project/deployment/batch_inference/notebooks/BatchInference"
  }
  ...
}
```

The example above defines a Databricks job with name `staging-my-mlops-project-batch-inference-job`
that runs the notebook under `my-mlops-project/deployment/batch_inference/notebooks/BatchInference.py` to regularly apply your ML model
for batch inference. 
